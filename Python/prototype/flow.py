...
def testing(loop:bool, max_exp:int):
    ...
    if loop:
        for i in range(1,max_exp+1):
            ...
            ntt = scheduling(in_NTT.copy(), N, q, psi, w_8th, w_4th)
        ...
        if match: print(f'[INFO] NTT Match Uniquely until {2**(max_exp)} point')
...
#SCHEDULER
def staging(point,A, N, q, psi, w_8th, w_4th,stage,radix_usage):
    if (point == 1):
        print("done")
    else:
...
def scheduling(A, N, q, psi, w_8th, w_4th):
    sched_order = [[0,0],[0,0],[0,0]] #buffer for scheduling radix
    radix_usage =[0,0,0] #number of times the radix is called (starting from radix 8 to 2)
    staging(N,A, N, q, psi, w_8th, w_4th,0,radix_usage)
